...
describe('Expiration Estimation test:', () => {
  ...
  const lowerResultTable: [string, number, string][] = testEntryMapKeys.map((key) => [
    key,
    testEntryMap[key][0],
    testEntryMap[key][1],
  ]); // Take lower expected result from tuple
  const upperResultTable: [string, number, string][] = testEntryMapKeys.map((key) => [
    key,
    testEntryMap[key][0],
    testEntryMap[key][2],
  ]); // Take upper expected result from tuple

  describe.each(lowerResultTable)(
    'getNextLowerExpirationFromSeconds for %s with seconds value %i',
    (durationName, inputSeconds, expectedEpiration) => {
  ...
  describe.each(upperResultTable)(
    'getNextHigherExpirationFromSeconds for %s with seconds value %i',
    (durationName, inputSeconds, expectedEpiration) => {
      test(`returns value ${expectedEpiration}`, () => {
...
describe('Malformed Requests test', () => {
  ...
  test.concurrent(
    'posting Myst with wrong language will use valid default language and successfully create Myst',
    async () => {
  ...
  test.concurrent(
    "posting Myst with code value being undefined and valid language and expiration creates myst with 'undefined' as code",
    async () => {
